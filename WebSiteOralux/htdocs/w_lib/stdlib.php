<?php rcs_id('$Id: stdlib.php,v 1.2 2004/12/30 00:14:16 gcasse Exp $');
   ...
   */
   ...
   // I couldn't move this to w_lib/config.php because it wasn't declared yet.
   $stack = new Stack;
   ...
   */
   ...
   function GeneratePage($template, $content, $name, $hash)
   {
	 ...
	 if ($condition) {
      ...
      $aLang=getLang();
      _dotoken('PAGETITLE', htmlspecialchars(GetFullName($name)), $page);
...
//        $page=preg_replace ("/<a href=\"http:\/\/(?!oralux.org)[^>]*>[^<]*<\/a>/i", "", $page);
//        $page=preg_replace ("/http:\/\/(?!(oralux.org)(php)[^ ]*/i", " ", $page);
