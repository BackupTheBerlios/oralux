...
/* {{{ proto resource dio_open_stdin

   the input mode is such that one byte is read as soon as typed (non canonical, +...) */
...
/* {{{ proto string dio_read
   ...
   Read 1 byte from the stdin and return it in a string */
PHP_FUNCTION(dio_read_stdin)
{
	...
	data = emalloc(bytes + 1);
	res = read(STDIN_FILENO, data, bytes);
	if (res <= 0) {
		efree(data);
		RETURN_NULL();
	}
				...
				if (*data==0x5b)
					{
						...
						char c=*data;
						if ((c==0x41) // Up arrow key
							||(c==0x42)) 
							{
								bytes=3;
								data = erealloc(data, bytes + 1);
								data[0]=0x1B;
								data[1]=0x5b;
								data[2]=c;
							}
	...
	RETURN_STRINGL(data, bytes, 0);
}

/* }}} */


/* {{{ proto resource dio_close_stdin

   Mostly reinstall the previous input mode */
